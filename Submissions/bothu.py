...
class Script:
    ...
    def get_move(self, player, enemy, player_projectiles, enemy_projectiles):
        ...
        enemyprimarycd = get_primary_cooldown(enemy)
        enemysecondcd = get_secondary_cooldown(enemy)
        ...
        for move in possible_moves:
            move_scores[move] = calculate_heuristic_score(move, distance, enemystun_time, enemyhp, enemyprimarycd, enemysecondcd)
...
# Function to calculate heuristic score for a move

def calculate_heuristic_score(move, distance, enemystun_time, enemyhp, enemyprimarycd, enemysecondcd):
    score = 0
    
    # Calculate score based on enemy HP
    score += hp_weight * (100 - enemyhp)
    
    # Calculate score based on distance
    score += distance_weight * (7 - distance)
    
    # Calculate score based on stun duration
    score += stun_weight * enemystun_time
    
    # Calculate score based on cooldowns
    if move == PRIMARY:
        score -= cooldown_weight * enemyprimarycd
    elif move == SECONDARY:
        score -= cooldown_weight * enemysecondcd
